...
class SellAnAlgoPage extends Component {
    ...
    onSubmit = async e => {
        ...
        const { name, image, description, howToUse, tags, price, sourcecode } = e.target;
        ...
        const algoSourceCode = new FormData();
        ...
        await axios.post( '/profile-img-upload', productImage, {
            headers: {
        ...
        await axios.post( '/algo-source-upload', algoSourceCode, {
            headers: {
                'accept': 'application/json',
                'Accept-Language': 'en-US,en;q=0.8',
...
// If not the given file type
        ...
        axios.post('/algos/save', productData)
        .then((response) => {
    ...
    render() {
        return (
            <HeaderSideBar title='Sell an Algo' addMargin={false} className='__Sell-An-Algo-Page'>
                <div className='__inline-block'>
                    <div className='__mt-s __dark-grey-text __mb-4'>Have a stock algorithm you want to sell? Fill out the fields below, submit your algo, and start earning money as soon as it’s approved!</div>
                    <form className='__flex-stretch' onSubmit={this.onSubmit}>
                        <div className='__form-left-panel __f1'>
                            <label className='__mb-3 __block'>
                                ...
                                <div className='__alert'>Name must be minimum 3 characters long!</div>
                            </label>
                            <div className='__h4 __mb-1 __bold'>Image</div>
                            <InputFile accept="image/x-png,image/gif,image/jpeg" className='__mb-3' onChange={e => console.log(e)} name='image' />
                            <label className='__mb-2 __block'>
                                <div className='__h4 __bold __mb-1'>Description</div>
                                <textarea className='__input-field-small __input' rows="5" name='description' placeholder='What is the purpose of this algo?…'></textarea>
                            ...
                            <label className='__block'>
                                ...
                                <textarea name='howToUse' className='__input-field-small __input' rows="5" placeholder='How would someone use this algo?…'></textarea>
                                <div className='__small __dark-grey-text __right'>Min. 50 characters</div>
                            </label>
                        </div>
                        <div className='__mb-a __right-form-part __f1 __mr-1'>
                            <div>
                                ...
                                <SelectTag name='tags' onChange={elm => console.log(elm.values)} />
                            </div>
                            <div className='__mt-3 __mb-25'>
                                <div className='__h4 __bold'>Price</div>
                                <div className='__dark-grey-text __small __mt-sm'>You’ll keep 90% of your earnings. The other 10% <br /> helps cover customer support and web maintenance.</div>
                                <PriceInput value='One-time purchase' name='price' onChange={(e, purchaseType) => console.log(e, purchaseType)} checked />
                                <PriceInput value='Monthly subscription' name='price' onChange={(e, purchaseType) => console.log(e, purchaseType)} />
                            </div>
                            <div>
                                <div className='__h4 __bold'>Upload Algorithm Source File</div>
                                <div className='__dark-grey-text __small __mt-sm __mb-15'>Acceptable formats: HTML, CSS, C++, <br /> Javascript, Go, Node, React, and Python</div>
                                <InputFile name='sourcecode' onChange={e => console.log(e)} />
                            </div>
                            <div className='__flex-stretch __mt-3'>
                                <div>
                                    <button className='__btn __pl-3 __pr-3'>Submit for Approval</button>
                                    <div className='__small __mt-s __dark-grey-text'>An algo typically takes 5-10 business <br /> days to be reviewed and approved</div>
                                </div>
                                <div className='__green-text __ml-2 __mt-s __pointer' onClick={this.props.history.goBack}>Cancel</div>
                            </div>
                        </div>
                    </form>
                </div>
...
class SelectTag extends Component {
    ...
    render() {
        return (
            <Fragment>
                <input type='hidden' name={this.props.name} ref='input' className='__input-field-small' />
                <div className='__wrap __chips __sb __select-tags'>
                    {this.state.tags.map(({ tag, active }, index) => <span className={`__chip ${active ? '__primary' : ''}`}
                        key={index}
    ...
    render() {
        ...
        return (
            <label className='__flex-stretch __mt-1 __price-input-container'>
                <input type='radio' name={name} value={value} className='__input-field-radio' defaultChecked={checked} ref='input' />
                <div className='__ml-1 __input-details'>
                    <div className='__bold __mb-s'>{value}</div>
                    <div className='__flex __price-details'>
                        <input className='__price-input-field __input-field-small __input' ref='__price' type='number' value={price} onChange={this.onChange} />
                        <span className={`__small __dark-grey-text __ml-sm __mr-2 ${value === 'One-time purchase' ? '__hide' : ''}`}>/month</span>
                        <div>
                            <span className='__small __dark-grey-text'>
                                What you’ll earn <br /> {value === 'One-time purchase' ? 'per sale' : 'per month'}:
                            </span> {eightper > 0 && `$${eightper}`}
                        </div>
                    </div>
