...
class ProductPaymentPanel extends Component {
    ...
    render() {
        ...
        return (
            <Fragment>
                {isRemoveAlgoPopUp && <RemoveAlgoPopUp close={this.hideRemoveAlgoPopUp} />}
                {isPaymentPopUp && <StripePaymentPopup onSubmit={this.onSubmit} close={this.hidePaymentPopUp} />}
                {isConfirmPaymentPopup && <ConfirmPaymentPopUp algoData={myAlgo._id} onConfirmation={this.onConfirmation} close={this.hideConfirmPaymentPopUp} />}
                    ...
                    <div className='__lightgrey-2 __payment-wrapper __scrollbar __p-4 __col __sb'>
                        <div className={isPurchased ? '__col __f1' : ''}>
                            ...
                            <div className='__card __mt-2'>
                                <div className="__small __dark-grey-text __mb-sm">{this.props.priceDetail.type}</div>
                                <div className='__h3 __bold'>${this.props.priceDetail.price}</div>
                            </div>
                            {isMyAlgo && <div className='__dark-grey-text __mt-2 __mb-4'>
                                <div className='__mb-1'>10 purchases</div>
                                <div>Approved May 20, 2020</div>
                            </div>}
                        </div>
                        {isMyAlgo && <div className='__flex __mt-a'>
                            <button className='__f1 __btn __mr-1' onClick={this.showRemoveAlgoPopUp}>Remove</button>
                        ...
                        <div className='__col'>
                            ...
                            {!card && !isMyAlgo && !isPurchased && <button onClick={this.showPaymentPopUp} className='__btn __p-s __flex __mt-a __p-sm __mt-4'>
                                <span className='__f1'>Add Payment</span> <span className='__h1 __ml-a __mr-sm'>+</span>
                            </button>}
                            {!isPurchased && !isMyAlgo && <button className='__p-sm  __mt-2 __btn __flex' disabled={card ? false : true} onClick={this.showConfirmPaymentPopUp}>
